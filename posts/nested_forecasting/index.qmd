...
This file runs a nested forecasting approach on top 10 Australian domestic routes data prior to covid. This is just an exploratory exercise and remains a work in progress.
...
### Training Forecast Accuracy by best
...
Not a great performance against actuals, post covid in Feb 2020, but that is as expected! Pre-covid prediciton looks good.
...
Obviously the pre and post covid volumes are much different so a lot of measures are not appropriate, but all lool reasonable pre-covid and horrible thereafter, as expected.
...
refit_model_aggr_acc_data %>%
  ...
  mutate(actual_prediction = 
    case_when(
      ...
      date              <= end_precovid ~ "actual_precovid_prediction",
      date              <= max_pred     ~ "actual_postcovid_prediction",
